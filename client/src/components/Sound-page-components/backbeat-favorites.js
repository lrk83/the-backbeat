   ...
   useEffect(() => {
    ...
    return (
        <div className="three-suggestions" data-aos="fade-in" data-aos-delay="100" data-aos-duration="1500">
            <Header as='h2' id= "top-creatives-title">Backbeat Favorites</Header>
            <Header as='h4' id="secondary-creatives-title">Some special packs, hand picked by the backbeat team</Header>
            {sortedSkillData[0] &&
            <Container className="three-suggestions-container">
                {cardsToShow.map((number)=> 
                    <Link key={number} className="three-suggestions-link" to={`/sounds/single-sound/${sortedSkillData[number]._id}`}>
                        <Card>
                            <Image src={sortedSkillData[number].image} wrapped ui={false} />
                            <Card.Content>
                                <Card.Header>{sortedSkillData[number].name}</Card.Header>
                            </Card.Content>
                        </Card>   
                    </Link>
                )}
...
export default BackBeatFavorites;